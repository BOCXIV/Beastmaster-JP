...
public sealed unsafe class BeastmasterCrucibleItemService
{
    ...
    public bool TryUseBestRecoveryItem(IBattleChara player, DateTime now, out ushort itemId)
    {
        ...
        {
            LastFailureReason = now < nextUseUtc ? "クルーシブルアイテムリキャスト中" : "自身が戦闘不能または HP が 0";
            return false;
        }
            ...
            || (int)director->InstanceContentType != 22)
        {
            LastFailureReason = actionManager == null ? "ActionManager が利用不可"
                : actionManager->AnimationLock > 0f ? "アクション実行硬直中"
        ...
        foreach (var recoveryItemId in RecoveryItemPriority)
        {
                ...
                || !CanUseOnSelf(recoveryItemId, self))
            {
                LastFailureReason = displaySlot < 0 ? $"魔獣回復薬 {recoveryItemId} のクルーシブルスロットが見つかりません"
                    : getUseStatus((byte*)director, inventorySlot, 0) != 0 ? $"魔獣回復薬 {recoveryItemId} のゲーム内ステータスが利用不可"
        ...
        LastFailureReason = "魔獣回復薬 78、77、76 が存在しないか、現在使用できません";
        return false;
    }
    ...
    public bool TryUseCrucibleItemOnTarget(BeastmasterCrucibleItemType itemType, IBattleChara player, IBattleChara target, DateTime now, out ushort itemId)
    {
        ...
        if (FangItemPriority.Length == 0)
            LastFailureReason = "牙のIDが設定されていません";
        return false;
    }
    ...
    public unsafe bool TryUseCrucibleItemOnTarget(ushort itemId, IBattleChara target, DateTime now)
    {
        ...
        {
            LastFailureReason = now < nextUseUtc ? "クルーシブルアイテムリキャスト中" : "ターゲットが戦闘不能または HP が 0";
            return false;
        }
            ...
            || getUseStatus((byte*)director, inventorySlot, 0) != 0)
        {
            LastFailureReason = displaySlot < 0 ? $"クルーシブルアイテム {itemId} のスロットが見つかりません"
                : $"クルーシブルアイテム {itemId} のゲーム内ステータスが利用不可";
            return false;
        }

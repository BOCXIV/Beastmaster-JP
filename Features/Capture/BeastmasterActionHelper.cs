...
public static class BeastmasterActionHelper
{
        ...
        bool useAdjustedActionId = false)
    {
        if (actionId == 0)
        {
            return new(0, "-", false, "無効なActionId");
        }
        ...
        {
            return new(0, $"アクション {actionId}", false, $"ActionIdが存在しません（{actionId}）");
        }

        var actionManager = ActionManager.Instance();
        if (actionManager == null)
        {
            return new(actionId, action.Name.ExtractText(), false, "ActionManager利用不可");
        }
        ...
        {
            return new(0, action.Name.ExtractText(), false, "置き換え後のアクションID取得不可");
        }

        if (!actionSheet.TryGetRow(resolvedActionId, out var resolvedAction))
        {
            return new(0, action.Name.ExtractText(), false, $"置き換え後のActionIdが存在しません（{resolvedActionId}）");
        }
        ...
        return actionStatus == 0
            ? new(resolvedActionId, actionName, true, "実行可能")
            : new(resolvedActionId, actionName, false, $"現在使用不可（コード {actionStatus}）");
    }

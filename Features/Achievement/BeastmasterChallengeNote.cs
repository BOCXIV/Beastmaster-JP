    ...
    public static IReadOnlyList<BeastmasterChallengeNoteEntry> GetBeastArenaEntries()
    {
        ...
        cachedEntries = sheet
            .Where(row => row.RowId != 0
                && (row.Name.ExtractText().Contains("闘獣", StringComparison.Ordinal)
                    || row.Name.ExtractText().Contains("魔獣", StringComparison.Ordinal)
                    || row.Name.ExtractText().Contains("闘獣練", StringComparison.Ordinal)
                    || row.Description.ExtractText().Contains("闘獣練", StringComparison.Ordinal)
                    || row.Description.ExtractText().Contains("魔獣", StringComparison.Ordinal)))

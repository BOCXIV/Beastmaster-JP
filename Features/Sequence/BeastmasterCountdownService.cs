...
public sealed class BeastmasterCountdownService : IDisposable
{
    ...
    private static readonly BeastmasterCountdownSnapshot DisabledSnapshot
        = new(false, false, 0f, 0, "自動出力停止中（カウントダウン監視停止）");
    ...
    public void StartCustomCountdown(float seconds)
    {
        ...
        {
            DalamudApi.ChatGui.Print("[魔獣使いアシスト] カウントダウン秒数は 1〜3600 の間で指定してください。");
            return;
        }

        if (!configuration.AutoCaptureEnabled)
        {
            DalamudApi.ChatGui.Print("[魔獣使いアシスト] 先に自動出力を有効にしてください（/魔獣使い 出力）。");
            return;
        }

        if (DalamudApi.PlayerState.ClassJob.RowId != BeastmasterClassJobId)
        {
            DalamudApi.ChatGui.Print("[魔獣使いアシスト] 現在のクラス・ジョブは魔獣使いではありません。");
            return;
        }
        ...
        var snapshot = new BeastmasterCountdownSnapshot(true, true, seconds, 0, $"カスタムカウントダウン {seconds:0.#} 秒");
        Snapshot = snapshot;

        DalamudApi.ChatGui.Print($"[魔獣使いアシスト] カスタムカウントダウン {seconds:0.#} 秒を開始しました。スキルシーケンスの実行を開始します。");
    }
    ...
    public void CancelCustomCountdown()
    {
        ...
        Snapshot = new(false, false, 0f, 0, "カスタムカウントダウン中止");
        DalamudApi.ChatGui.Print("[魔獣使いアシスト] カスタムカウントダウンを中止しました。");
    }
    ...
    private void OnFrameworkUpdate(IFramework framework)
    {
        ...
        if (customCountdownActive)
        {
            ...
            if (customCountdownRemaining <= 0f)
            {
                ...
                Snapshot = new(true, false, 0f, 0, "カスタムカウントダウン完了");
                DalamudApi.ChatGui.Print("[魔獣使いアシスト] カスタムカウントダウンが完了しました。戦闘開始待機中。");
            }
            else
            {
                Snapshot = new(true, true, customCountdownRemaining, 0, $"カスタムカウントダウン {customCountdownRemaining:0.#} 秒");
            }

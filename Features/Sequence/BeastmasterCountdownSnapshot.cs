    ...
    string Status)
{
    public static unsafe BeastmasterCountdownSnapshot Read()
    {
        ...
        {
            return Unavailable("Framework または UIModule が利用不可");
        }

        var agentModule = framework->UIModule->GetAgentModule();
        if (agentModule == null)
        {
            return Unavailable("AgentModule が利用不可");
        }

        var agent = agentModule->GetAgentByInternalId(AgentId.CountDownSettingDialog);
        if (agent == null)
        {
            return Unavailable("CountDownSettingDialog Agent が利用不可");
        }
        ...
        {
            return Unavailable($"カウントダウン数値異常：{timer}");
        }

        var active = countdown->Active != 0;
        return new(true, active, active ? timer : 0f, countdown->Initiator, "正常");
    }

...
public sealed class BeastmasterRuleService
{
        ...
        DateTime now)
    {
        ...
        var message = $"ルールセット「{ruleSet.Name}」第 {ruleIndex + 1} ルール「{rule.Name}」合致：{matchReason}；{availability.ActionName}（{availability.ActionId}）を実行リクエスト";
    ...
    private void Fail(
        ...
        DateTime now)
    {
        var message = $"ルールセット「{ruleSet.Name}」第 {ruleIndex + 1} ルール「{rule.Name}」合致：{matchReason}；{failureReason}のため通常ACRへフォールバック";
        RecordDiagnostic(message);
        if (configuration.RuleDiagnosticsEnabled
        ...
        TimeSpan interval)
    {
        ...
        lastChatKey = key;
        lastChatUtc = now;
        DalamudApi.ChatGui.Print($"[Beastmaster {DateTime.Now:HH:mm:ss}] {message}");
    }
}
